...
func (*FSCheckpoint) Usage() string {
	...
	Saves checkpoint containing filesystem modifications in all containers in the given container's sandbox to -image-path directory. -overlay2 must be enabled with media "self" or a directory, and only the upper layers of these overlays will be saved. Restore checkpoints using "runsc create -fs-restore-image-path=<path>".
`
}
