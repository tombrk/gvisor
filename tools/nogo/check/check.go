...
type importer struct {
	fset    *token.FileSet
	sources map[string][]string // key is full package path (including vendor directories).
	...
	mu    sync.Mutex
	cache map[string]*importerEntry // key is full package path (including vendor directories).
	...
	imports   map[string]*types.Package // key is full package path (including vendor directories).
}
...
func (i *importer) importPackage(pkgPath, dir string) (*types.Package, error) {
	if pkgPath == "unsafe" {
	...
	if dir != "" && len(i.roots) > 0 {
		// Vendor directories get first chance to statisfy import.
		//
		...
		for {
			fullPath := path.Join(dirPath, "vendor", pkgPath)
			if pkgExists(fullPath) {
...
func SplitPackages(srcs []string, srcRootPrefix string) (map[string][]string, error) {
	pkgs, err := splitPackages(srcs, srcRootPrefix)
	if err != nil {
		return nil, err
	}
